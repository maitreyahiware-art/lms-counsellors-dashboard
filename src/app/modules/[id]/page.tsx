    ...
    return (
        ...
        >
            <AnimatePresence>
                ...
                {/* ── General Modules Complete Transition Screen ── */}
                {showGeneralCompleteScreen && nextModule && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[130] flex items-center justify-center p-6 bg-[#0E5858]/95 backdrop-blur-xl"
                    >
                        <motion.div
                            initial={{ scale: 0.88, y: 30 }}
                            animate={{ scale: 1, y: 0 }}
                            transition={{ type: 'spring', stiffness: 240, damping: 24 }}
                            className="w-full max-w-2xl bg-white rounded-[3rem] overflow-hidden shadow-2xl relative"
                        >
                            {/* Top colour bar */}
                            <div className="h-2 w-full bg-gradient-to-r from-[#0E5858] via-[#00B6C1] to-[#FFCC00]" />

                            <div className="p-12 text-center">
                                {/* Trophy icon */}
                                <div className="w-20 h-20 bg-[#FFCC00]/10 rounded-[1.5rem] flex items-center justify-center mx-auto mb-8 shadow-lg">
                                ...
                                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest mb-6 border border-green-100">
                                    ...
                                    General Training Done
                                </div>

                                <h2 className="text-4xl font-serif text-[#0E5858] mb-4 leading-tight">
                                    Great job finishing the<br />Business Overview!
                                </h2>
                                <p className="text-gray-500 text-sm font-medium leading-relaxed max-w-md mx-auto mb-10">
                                    You have covered how Balance Nutrition works, our programs, tools, and what sets us apart. Now it's time to go deeper — into training built specifically around your role.
                                </p>

                                {/* Divider */}
                                <div className="flex items-center gap-4 mb-10">
                                    <div className="flex-1 h-px bg-gray-100" />
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#00B6C1]">What's coming next</p>
                                    <div className="flex-1 h-px bg-gray-100" />
                                </div>

                                {/* Next module preview card */}
                                {nextModule.id === 'educators' ? (
                                    <div className="bg-[#0E5858] rounded-[2rem] p-8 text-left text-white mb-10">
                                        <div className="flex items-start gap-5">
                                            <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center flex-shrink-0">
                                                <BookOpen size={26} className="text-[#FFCC00]" />
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-[#FFCC00] mb-1">Educators Module</p>
                                                <h3 className="text-xl font-serif mb-2">Your Content Hub</h3>
                                                <p className="text-sm text-white/70 leading-relaxed">
                                                    This module is a content hub that breaks myths and builds solid knowledge — so you can confidently educate the people around you: peers, family, and clients. Use it to <strong className="text-white">find content easily</strong>, <strong className="text-white">build your knowledge</strong>, and <strong className="text-white">share it</strong> with the right people at the right time.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="bg-[#FAFCEE] rounded-[2rem] p-8 text-left border border-[#00B6C1]/10 mb-10">
                                        <div className="flex items-start gap-5">
                                            <div className="w-14 h-14 bg-[#00B6C1]/10 rounded-2xl flex items-center justify-center flex-shrink-0">
                                                <BookOpen size={26} className="text-[#00B6C1]" />
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-[#00B6C1] mb-1">Up Next</p>
                                                <h3 className="text-xl font-serif text-[#0E5858] mb-2">
                                                    {nextModule.title.replace(/^Module \d+:\s*/i, '')}
                                                </h3>
                                                <p className="text-sm text-gray-400 leading-relaxed">
                                                    {nextModule.id === 'module-3'
                                                        ? 'This is where you learn how to talk to clients — how to listen well, build trust, and guide them towards making a decision that feels right for them.'
                                                        : nextModule.id === 'module-4'
                                                        ? 'Get hands-on with the tools you will use every day — the counsellor dashboard, lead tracking, and how to manage your work smoothly.'
                                                        : nextModule.id === 'module-5'
                                                        ? 'Sharpen your selling skills — how to have confident conversations, handle questions, and help clients see the value in what we offer.'
                                                        : nextModule.description || 'Continue your role-based training journey.'
                                                    }
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <button
                                    onClick={() => {
                                        setShowGeneralCompleteScreen(false);
                                        router.push(`/modules/${nextModule.id}`);
                                    }}
                                    className="w-full py-5 bg-[#0E5858] text-white rounded-[1.5rem] font-bold text-base shadow-2xl hover:bg-[#00B6C1] transition-all flex items-center justify-center gap-3 group"
                                >
                                    Let's Go — Start {nextModule.title.replace(/^Module \d+:\s*/i, '')}
                                    <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                                </button>

                                <button
                                    onClick={() => setShowGeneralCompleteScreen(false)}
                                    className="mt-5 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-[#0E5858] transition-colors"
                                >
                                    Go back to this module
                                </button>
                            </div>
                        </motion.div>
                    </motion.div>
